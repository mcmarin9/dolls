...
const DollDetail: React.FC<DollDetailProps> = ({ doll, isOpen, onClose, onLoteClick }) => {
    ...
    useEffect(() => {
        const fetchDollLotes = async () => {
            ...
            setError(null);
            try {
                const response = await fetch(`http://localhost:5000/api/dolls/${doll.id}/lotes`);
        ...
        <div className="w-full">
            ...
            <div className="doll-detail">
                ...
                <div className="grid gap-2">
                    ...
                    <span className={`px-2 py-1 rounded-full text-sm ${getStatusStyle(doll.estado || '')}`}>
                        {doll.estado}
                    </span>
                </p>           {doll.comentarios && (
                    ...
                    {doll.precio_compra && (
                        <p><span className="font-semibold">Precio de compra:</span> {doll.precio_compra}€</p>
                    )}
                    {doll.precio_venta && (
                        <p><span className="font-semibold">Precio de venta:</span> {doll.precio_venta}€</p>
                    )}
                    ...
                    ) : dollWithLotes.lotes && dollWithLotes.lotes.length > 0 && (
                      <div className="mt-4">
                      ...
                      <div className="flex flex-wrap gap-2">
                          {dollWithLotes.lotes.map((lote) => (
                              <button
                                  ...
                                  className={`px-3 py-1 rounded-full text-sm transition-colors cursor-pointer ${getTypeStyle((lote.tipo || '').toLowerCase() as "compra" | "venta")}`}
                              >
                                  {lote.nombre}
                              </button>
                          ))}
